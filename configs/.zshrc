...
# Цвета приглашения. Файл везёт этот же репозиторий (configs/.p10k.zsh),
# кладёт его install_configs. Подключается последним из всего, что красит:
# p10k перечитывает POWERLEVEL9K_* на каждом приглашении, поэтому заданное
# в нём не перекрывается ничем выше. Условие оставлено на случай, когда
# конфиг снесли руками, — тогда p10k возьмёт свои цвета, а не упадёт.
