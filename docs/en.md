...
- `.p10k.zsh` - colours for the powerlevel10k prompt: 151 `POWERLEVEL9K_*` parameters covering all 87 segments and all their states, every value from the Digitable palette. Installed to `~/.p10k.zsh` and sourced from `.zshrc` as the last of the lines that paint
- `.vimrc` - Vim settings
...
- Configurations for various utilities in `.config/`:
  ...
  - `tmux` - session configurations plus `battery.sh`, the charge block for the bottom bar. The dracula plugin's own battery widget paints one colour at every percentage, so the block is ours: the colour reports the level (below 20% red, below 30% orange, below 60% yellow, above that green), and cyan while on mains power. The block reads its colours from tmux's `@dracula-colors`; not one of them is written in it as a number
...
### Palette
One set of seventeen colours paints Vim, bat, delta, Alacritty, tmux, bpytop, lf, vifm, eza, GNU ls, zsh's suggestion line, the powerlevel10k prompt and three Logseq plugins. It has a single source: the `theme/digitable.flang` specification, which declares the colours themselves, the mapping of the nine highlighting roles and the contrast thresholds, and carries the checks. `scripts/check-theme.sh` holds the tree to it: every colour in the configs must either be declared in the spec or be derived from it as a "colour at N% over the background" blend, and every declared colour must be used somewhere. Colour is written two ways in the tree - `#rrggbb` and the 24-bit ANSI form `38;2;R;G;B` that lf and `EZA_COLORS` use - and the check reads both. `LS_COLORS` is not written a second time: `.zshrc` derives it from lf's own colours file, whose format is exactly `LS_COLORS`, so the two lists cannot drift apart. The WCAG 2.1 contrast is computed by flang itself (`flang check theme/digitable.flang`); with no compiler on the machine the check honestly prints "not measured" rather than "ok" - and `flang` is one of the tools the installer offers, so a machine can be brought up to the point of measuring.

The shell prompt is drawn by powerlevel10k, and a single file paints it: `configs/.p10k.zsh`. No fork of the theme is needed, and that was measured rather than assumed: every configuration point of p10k is a `POWERLEVEL9K_*` parameter, and a segment of your own is written as a `prompt_<name>` function in the config itself through the public `p10k segment` command (`p10k help segment`) - upstream's own template ships exactly such a segment, `prompt_example` at line 1650 of `config/p10k-lean.zsh`. There is nothing to change in the theme's code, and a fork would commit us to tracking someone else's upstream forever for the same result.

Of the three styles, lean is the one taken. It gives segments no background at all (`POWERLEVEL9K_BACKGROUND` is empty; zero `BACKGROUND` parameters against rainbow's 111), so every character of the prompt sits on the terminal background, which here is `bg0`. That makes each colour's contrast exactly the spec's "colour contrast", already computed and already held to the threshold: the prompt adds no new "text on a surface" pair to the spec. Classic and rainbow would each require re-measuring every colour against every new surface. 151 colour values are set - more than the wizard's template, which has 118. The template says nothing about a merge conflict, the four vi modes, a read-only directory, ssh, the user or the host: there the colour is drawn from the terminal's own palette by name (`yellow1`, `blue`, `white`) or by ANSI index. Nor does it say that the state name of the aws, kubernetes and terraform segments comes from the value itself - from the profile or context name - so `POWERLEVEL9K_AWS_DEFAULT_FOREGROUND` paints exactly the profile named "default", and in production the prompt would be painted by something else. A run under a real zsh shows 152 colour parameters taken in (p10k derives two of them from the battery array) and not one value from outside the palette; not one of p10k's 87 segments is left without a colour of ours. Some of the gaps turned up in the run and in a sweep of the segment list, not in reading the template.

The colours are named as hexadecimal values, not as names like `red` and not as 256-colour indices. A name is resolved out of the terminal's palette by the terminal, not by the theme: under someone else's profile `red` is someone else's red. p10k understands hex itself, and on a 256-colour terminal the `zsh/nearcolor` module that `.zshrc` loads substitutes the nearest colour. The `p10k configure` wizard would overwrite this file - with its own 118 colours from the 256-colour palette and none of ours - so the config is edited by hand.

Battery charge shows up in two places - the block in tmux's bottom bar and the `battery` segment in the p10k prompt - and they share their thresholds. The number 20 is not invented: upower calls the charge low from there (`PercentageLow=20`), and macOS raises its first warning at the same point. p10k has no colours of its own for the battery: by default it asks for ANSI colour names, which means the terminal paints it. The four per-state colours (`POWERLEVEL9K_BATTERY_LOW_FOREGROUND` and its neighbours) are not set in `.p10k.zsh` at all - they beat the per-level colour, and it is the level that carries the thresholds: an array of a hundred elements paints the same four states, but by percentage. The thresholds are written three times (the spec, `sh`, `zsh` - neither shell reads the spec), and `check-theme.sh` holds the three together; the `--selftest` negative control shows that the comparison can fail.
