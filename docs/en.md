...
Battery charge shows up in two places - the block in tmux's bottom bar and the `battery` segment in the p10k prompt - and they share their thresholds. The number 20 is not invented: upower calls the charge low from there (`PercentageLow=20`), and macOS raises its first warning at the same point. p10k has no colours of its own for the battery - by default it asks for ANSI colour names, which means the terminal paints it rather than the theme; in `.zshrc` the colours are named as hexadecimal values and no longer depend on the terminal palette. The same place clears the per-state colours (`POWERLEVEL9K_BATTERY_LOW_FOREGROUND` and its neighbours): the p10k wizard writes 160, 70 and 178 into `~/.p10k.zsh`, and those beat the per-level colour - without clearing them the thresholds never show. The thresholds are written three times (the spec, `sh`, `zsh` - neither shell reads the spec), and `check-theme.sh` holds the three together; the `--selftest` negative control shows that the comparison can fail.
