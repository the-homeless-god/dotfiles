...
- `.digit/config.yaml` - настройки Digit CLI: локальный провайдер моделей с OpenAI-совместимым интерфейсом. Ставится в `~/.digit/config.yaml`; секреты живут в `.env` и здесь не хранятся
