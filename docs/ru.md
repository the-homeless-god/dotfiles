...
- `.vimrc` - настройки Vim
- `.vim/colors/digitable.vim` - цветовая схема Vim в палитре портала [Digitable Courses](https://courses.digitable.life). Ставится в `~/.vim/colors/`; подключается из `.vimrc` командой `colorscheme digitable`. Цвета взяты из токенов портала, раскладка по ролям (ключевое слово, строка, комментарий) - из его же единственной таблицы подсветки, поэтому код в Vim выглядит так же, как в блоке кода на странице портала. Каждая пара «текст / фон» проверена по WCAG 2.1, у каждой группы есть значение и для терминала на 256 цветов
