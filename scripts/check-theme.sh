...
read_palette() {
    ...
    local pal
    pal="$(awk '
        # Ёлочки написаны буквами, а не точкой: в локали C awk считает
        # байты, и точка совпала бы с половиной «. Тогда тело функции не
        # находится, палитра выходит пустой, и проверка краснеет на всём
        # дереве разом — ложная тревога вместо проверки.
